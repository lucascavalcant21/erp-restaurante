        ...
        return <article key={ficha.id} className={`rounded-2xl border p-3 transition ${ativo ? "border-emerald-300 bg-emerald-50/40" : "border-slate-200 bg-slate-50/50"}`}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-3xs font-bold uppercase text-slate-400">porções</span>
            </div>
          </div>
  ...
  return <div className="min-h-screen bg-[var(--surface)] pb-24 text-slate-800">
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-4 backdrop-blur sm:px-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3"><button onClick={() => router.back()} className="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-slate-50 text-slate-600"><ArrowLeft size={20}/></button><div><h1 className="text-2xl font-black tracking-tight">Produção do {nomeSetorAtivo}</h1><p className="text-xs font-bold text-slate-500">Lista separada · {unidadeInfo?.nome}</p></div></div>
    ...
    <main className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
      ...
      {loading ? <div className="p-16 text-center font-bold text-slate-400">Carregando fichas e estoque...</div> : setorAtivo === "cozinha" ? <SetorPlanejamento titulo="Produção da Cozinha" icone={ChefHat} cor="bg-emerald-700" fichas={fichasCozinha} plano={cozinha} colaboradores={colaboradores} alterar={(id, patch) => alterarPlano(setCozinha, id, patch)}/> : <SetorPlanejamento titulo="Produção do Bar" icone={GlassWater} cor="bg-blue-700" fichas={fichasBar} plano={bar} colaboradores={colaboradores} alterar={(id, patch) => alterarPlano(setBar, id, patch)}/>}
