...
export default function EtiquetasRapidas() {
    ...
    Promise.all([fetchEstoque(unidadeAtiva, setor), fetchProdutos(unidadeAtiva, setor), fetchFichas(unidadeAtiva, setor)]).then(([estoque, cardapio, fichas]) => {
      ...
      (estoque.data || []).forEach(produto => {
        ...
        mapa.set(produto.nome.toLocaleLowerCase("pt-BR"), {
          ...
          custo: numero(produto.custo_unitario || produto.preco_unit), origem: "Ingrediente",
        });
      });
      // Pré-preparo entra antes do cardápio: quando existir uma ficha base com
      // o mesmo nome de um item vendido, o que vale é o preparo.
      (fichas.data || []).filter(f => f.eh_base).forEach(ficha => {
        ...
        mapa.set(nome.toLocaleLowerCase("pt-BR"), {
          id: `base:${ficha.id || nome}`, nome,
          unidade: String(ficha.rendimento_unidade || "UN").toUpperCase(),
          custo: 0, origem: "Pré-preparo",
        });
      });
  ...
  // Separado por onde o produto vem: ingrediente cru, pré-preparo da casa ou
  // item do cardápio. Vale igual para cozinha e bar.
  const ORIGENS = ["Ingrediente", "Pré-preparo", "Cardápio"];
  const porOrigem = useMemo(() => {
    const grupos = new Map(ORIGENS.map(o => [o, []]));
    visiveis.forEach(produto => {
      const chave = grupos.has(produto.origem) ? produto.origem : "Ingrediente";
  ...
  if (!setor) return <div className="etq-inicio"><style>{ESTILOS}</style><div className="etq-topo"><button onClick={voltar}><ArrowLeft size={19} /> Voltar</button><button onClick={telaCheia}><Maximize2 size={18} /> Tela cheia</button></div><main><Tag size={52} /><h1>Etiquetas</h1><p>Escolha a área para começar.</p><div className="etq-setores"><button className="cozinha" onClick={() => setSetor("cozinha")}><ChefHat /> Cozinha <span>Voz ou manual · produtos e pré-preparos</span></button><button className="bar" onClick={() => setSetor("bar")}><GlassWater /> Bar <span>Voz ou manual · bebidas e pré-preparos</span></button></div></main></div>;

  if (!responsavel) return <div className="etq-pessoas" style={{ "--cor": setor === "bar" ? "#2563eb" : "#059669" }}><style>{ESTILOS}</style><div className="etq-pessoas-topo"><button onClick={voltar}><ArrowLeft size={19}/> Voltar</button><button onClick={telaCheia}><Maximize2 size={18}/> Tela cheia</button></div><main><div className="etq-pessoas-icone"><UserRound size={34}/></div><h1>Quem está etiquetando?</h1><p>{setor === "bar" ? "Bar" : "Cozinha"} · escolha seu nome uma vez para montar toda a fila.</p>{carregando ? <div className="etq-sem"><RefreshCw className="animate-spin"/> Carregando equipe...</div> : <div className="etq-pessoas-grid">{funcionarios.map(funcionario => <button key={funcionario.id} onClick={() => setResponsavelId(String(funcionario.id))}><span><UserRound size={24}/></span><div><strong>{funcionario.nome}</strong><small>{funcionario.cargo || "Funcionário"}</small></div></button>)}</div>}</main></div>;
  ...
  return <div className="etq-app" style={{ "--cor": setor === "bar" ? "#2563eb" : "#059669", "--suave": setor === "bar" ? "#eff6ff" : "#ecfdf5" }}>
    ...
    <main className="etq-conteudo">
      ...
      <div className="etq-busca"><Search size={20} /><input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar produto..." />{busca && <button onClick={() => setBusca("")}><X size={17} /></button>}</div>{carregando ? <div className="etq-sem"><RefreshCw className="animate-spin" /> Carregando...</div> : porOrigem.length ? porOrigem.map(([origem, lista]) => <section key={origem} className="etq-origem"><h3>{origem === "Pré-preparo" ? "Pré-preparos" : origem === "Cardápio" ? "Cardápio" : "Ingredientes"}<span>{lista.length}</span></h3><div className="etq-grid">{lista.map(produto => <button key={produto.id} onClick={() => abrirProduto(produto)}><span><Tag size={17} /></span><strong>{produto.nome}</strong><small>adicionar à fila</small></button>)}</div></section>) : <div className="etq-sem">Nenhum produto encontrado.</div>}
    </main>
