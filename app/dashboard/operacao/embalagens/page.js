         ...
         <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                 ...
                 <p className="text-subtle font-bold uppercase tracking-widest text-xs mt-1">Estoque separado de {setorNome}</p>
              </div>
            </div>
         </div>
      </div>
          ...
          <>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-4">
                ...
                {embalagens.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {embalagens.map(emb => (
                      <div key={emb.id} className="bg-card border border-line rounded-3xl p-5 shadow-sm flex flex-col group relative">
            ...
            {aba === 'saida' && (
              <div className="animate-in fade-in slide-in-from-bottom-4 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                ...
                <div className="bg-card border border-line rounded-[32px] p-5 sm:p-8 shadow-xl">
                ...
                <div className="bg-slate-50 border border-line rounded-[32px] p-6 h-full flex flex-col">
                   ...
                   <div className="flex-1 overflow-y-auto max-h-[500px] pr-2 space-y-3 custom-scrollbar">
                      {historico.length === 0 ? (
                         <div className="text-center text-subtle font-bold py-10">Nenhuma retirada registrada ainda.</div>
                      ) : (
                         historico.map(h => (
                            <div key={h.id} className="bg-card p-4 rounded-2xl border border-line shadow-sm">
                ...
                {compras.length === 0 ? (
                  ...
                  <div className="rounded-2xl overflow-x-auto shadow-md border border-line">
                    <div className="bg-gradient-to-r from-slate-800 to-slate-700 px-6 py-4 grid grid-cols-[1fr_120px_140px_140px] gap-4 items-center min-w-[680px]">
                      <span className="text-2xs font-bold uppercase tracking-widest text-dim">Item</span>
                      <span className="text-2xs font-bold uppercase tracking-widest text-dim text-center">Saldo</span>
                      <span className="text-2xs font-bold uppercase tracking-widest text-dim text-center">Mínimo Ideal</span>
                      <span className="text-2xs font-bold uppercase tracking-widest text-dim text-right">Faltam</span>
                    </div>
                    <div className="bg-card divide-y divide-slate-100">
                      {compras.map(emb => (
                        <div key={emb.id} className="px-5 py-4 grid grid-cols-[1fr_120px_140px_140px] gap-4 items-center min-w-[680px] hover:bg-rose-50/40 transition-all duration-150">
                          ...
                          <div className="text-center">
                            <span className="font-black text-subtle text-xl">{emb.quantidade_minima}</span>
                          </div>
