    ...
    <div className="flex flex-col h-[calc(100dvh-72px)] bg-elevated overflow-hidden font-sans">
        ...
        <div className="bg-gradient-to-r from-orange-500 to-rose-500 text-white px-3 sm:px-4 py-3 flex flex-wrap items-center justify-between gap-2 z-50 shadow-lg animate-pulse shrink-0">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-card text-orange-600 font-bold text-xs px-4 py-2 rounded-xl hover:bg-orange-50 transition-colors shadow-sm"
            >
         ...
         <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            ...
            <div className="hidden sm:block">
               ...
               <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className="text-3xs font-bold text-dim uppercase">Turno Aberto</span>
               </div>
            </div>
            
            <div className="sm:ml-4 lg:ml-8 bg-black/20 p-1 rounded-xl flex gap-1 items-center">
               <button onClick={() => setAbaAtiva('salao')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap ${abaAtiva === 'salao' ? 'bg-card text-fg shadow-sm' : 'text-dim hover:text-white'}`}>
                  <Users size={14} /> Salão (Mesas)
               </button>
               <button onClick={() => setAbaAtiva('balcao')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap ${abaAtiva === 'balcao' ? 'bg-card text-fg shadow-sm' : 'text-dim hover:text-white'}`}>
                  <ShoppingBag size={14} /> Balcão Rápido
               </button>
               <button onClick={() => setAbaAtiva('ifood')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap ${abaAtiva === 'ifood' ? 'bg-card text-fg shadow-sm' : 'text-dim hover:text-white'}`}>
                  <Utensils size={14} /> iFood
               </button>
               <button onClick={() => setAbaAtiva('cardapio')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs font-bold transition-all flex items-center gap-2 whitespace-nowrap ${abaAtiva === 'cardapio' ? 'bg-card text-fg shadow-sm' : 'text-dim hover:text-white'}`}>
         ...
         {abaAtiva === 'salao' && mesaAtiva && pedidoAtivo ? (
            <>
               ...
               <div className="w-full lg:w-[320px] h-[42%] lg:h-auto bg-card border-b lg:border-b-0 lg:border-r border-line shadow-xl flex flex-col shrink-0 z-20">
                  ...
                  <div className="p-4 bg-card border-t border-line shrink-0 z-10">
            ...
            <>
                     ...
                     <div className="flex flex-col h-full overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                           <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                              {produtosFiltrados.map(prod => (
                                 <button key={prod.id} onClick={() => handleSelecionarProdutoBalcao(prod)} className="bg-card rounded-xl p-3 border border-line shadow-sm hover:shadow-md hover:border-emerald-500 transition-all text-left flex flex-col group h-full">
               ...
               {abaAtiva !== 'salao' && (
                   <div className="w-full lg:w-[400px] h-[42dvh] lg:h-auto bg-card border-t lg:border-t-0 lg:border-l border-line shadow-2xl flex flex-col shrink-0 z-20">
                     ...
                     <div className="flex-1 overflow-y-auto p-3 bg-slate-50/30 custom-scrollbar space-y-2">
                        {carrinho.length === 0 ? (
                           ...
                           return (
                           <div key={item.uniqueId} className="bg-card p-3 rounded-xl border border-line shadow-sm flex flex-col gap-2 relative group">
                              ...
                              <div className="flex justify-between items-start">
                                 <div>
                                    <span className="font-bold text-fg-soft text-xs">{item.nome_produto}</span>
      ...
      {modalMod && prodModAtual && (
         <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
             <div className="bg-card rounded-[32px] p-5 sm:p-8 w-full max-w-md max-h-[calc(100dvh-1rem)] overflow-y-auto shadow-2xl animate-in zoom-in-95">
               ...
               <div className="space-y-3 my-8 max-h-[40vh] overflow-y-auto custom-scrollbar pr-2">
                  {prodModAtual.modificadores.map((mod, i) => {
                     ...
                     return (
                        <button key={i} onClick={() => toggleMod(mod)} className={`w-full flex justify-between items-center p-4 rounded-2xl border-2 transition-all ${selecionado ? 'border-emerald-500 bg-emerald-50' : 'border-line bg-card hover:border-slate-300'}`}>
                           <div className="flex items-center gap-3">
         ...
         <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-card rounded-3xl p-6 w-full max-w-md shadow-2xl">
               <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => setModalLancar(false)} className="text-subtle hover:text-red-500"><X size={20}/></button>
               </div>
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm p-2 sm:p-4 animate-in fade-in">
             <div className="bg-card rounded-[24px] sm:rounded-[32px] w-full max-w-4xl shadow-2xl flex flex-col md:flex-row overflow-y-auto md:overflow-hidden max-h-[calc(100dvh-1rem)] sm:max-h-[95vh]">
               ...
               <div className="w-full md:w-1/3 bg-slate-50 border-b md:border-b-0 md:border-r border-line p-4 sm:p-6 overflow-y-visible md:overflow-y-auto custom-scrollbar">
                  ...
                  <div className="space-y-4">
                     {abaAtiva === 'salao' && (
                     <div className="flex items-center justify-between bg-card p-3 rounded-xl border border-line shadow-sm">
                        <label className="text-xs font-bold text-fg-soft">Taxa de Serviço (10%)</label>
                        <button type="button" onClick={() => setAplicarDezPorcento(!aplicarDezPorcento)} className={`w-12 h-6 rounded-full transition-colors relative ${aplicarDezPorcento ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                           <div className={`w-4 h-4 bg-card rounded-full absolute top-1 transition-transform ${aplicarDezPorcento ? 'translate-x-7' : 'translate-x-1'}`}></div>
                        </button>
                     </div>
                     )}
                     <div className="grid grid-cols-2 gap-2">
                     ...
                     <div className="pt-2 border-t border-line">
                        ...
                        {cupomAplicado ? (
                           <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 p-3 rounded-xl">
                              ...
                              <button type="button" onClick={() => { setCupomAplicado(null); setDescontoPerc(""); setDescontoRs(""); }} className="text-red-500 hover:text-red-700 font-bold text-xs bg-card px-2 py-1 rounded shadow-sm">Remover</button>
                           </div>
                        ) : (
                           <div className="flex gap-2">
                              <input type="text" value={cupomDigitado} onChange={e=>setCupomDigitado(e.target.value.toUpperCase())} onKeyDown={checkBipeCupom} placeholder="CÓDIGO" className="flex-1 px-4 py-2.5 rounded-xl border border-slate-300 outline-none font-black text-fg-soft uppercase" />
               ...
               <div className="flex-1 p-4 sm:p-8 flex flex-col bg-card min-w-0">
                  ...
                  <div className="flex justify-between items-center bg-slate-800 text-white rounded-2xl p-5 mb-6 shadow-md">
                     <div>
                        <p className="text-xs font-bold text-subtle uppercase tracking-widest">Total Líquido</p>
                     ...
                     <div className="text-right">
                        <p className="text-xs font-bold text-subtle uppercase tracking-widest">Restante</p>
                        ...
                        {pagamentos.map((p) => (
                            <div key={p.id} className="flex flex-col sm:flex-row gap-3 sm:items-center bg-slate-50 p-3 rounded-xl border border-line">
                              <select value={p.forma} onChange={e => {
                                  ...
                                  setPagamentos(n);
                              }} className="w-full sm:w-1/2 px-3 py-2 bg-card border border-slate-300 rounded-lg font-bold text-fg-soft outline-none">
      ...
      {modalRecibo && dadosRecibo && (
         <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
             <div className="bg-card rounded-2xl p-4 sm:p-6 w-[min(320px,calc(100vw-1rem))] max-h-[calc(100dvh-1rem)] overflow-y-auto shadow-2xl flex flex-col font-mono relative">
               ...
               <div className="text-xs mb-2">
                  <div className="flex justify-between font-bold border-b border-line pb-1 mb-1">
               ...
               <div className="text-center text-3xs mt-4 pt-3 border-t border-slate-800">
                  ...
                  <p className="text-subtle mt-1">Volte sempre</p>
               </div>
      ...
      {modalGestaoGarcons && (
         <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-card rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
               <div className="p-6 border-b border-line-soft bg-slate-800 text-white flex justify-between items-center">
                  <div><h2 className="text-xl font-black">Gerenciar Garçons</h2><p className="text-sm text-subtle">Cadastre a equipe</p></div>
                  <button onClick={() => setModalGestaoGarcons(false)} className="text-subtle hover:text-white"><X size={24} /></button>
               </div>
            ...
            <div className="bg-[#4A6487] p-4 text-white flex justify-between items-center shadow-md">
             ...
             <div className="flex-1 overflow-y-auto p-4 sm:p-10 bg-[#F1F5F9] flex flex-col items-center">
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full max-w-5xl">
                  ...
                  }} className="bg-card border-2 border-dashed border-emerald-400 hover:bg-emerald-50 rounded-2xl p-6 flex flex-col items-center justify-center min-h-[160px] transition-colors group">
                  ...
                  {pedidosDaMesa.map(pedido => (
                     <button key={pedido.id} onClick={() => {
                        ...
                        setModalListaComandas(false);
                     }} className="bg-card border border-line rounded-2xl p-6 shadow-sm hover:shadow-lg hover:border-blue-400 transition-all flex flex-col items-center text-center">
                        ...
                        <span className="font-black text-fg-soft text-lg">{pedido.identificacao || 'Sem Ident'}</span>
                        <span className="text-xs font-bold text-subtle uppercase tracking-widest mt-1">R$ {(pedido.pedidos_itens?.reduce((acc, it) => acc + (it.quantidade * it.valor_unitario), 0) || 0).toFixed(2).replace('.',',')}</span>
                     </button>
                  ))}
               </div>
            </div>
                ...
                <div className="w-full max-w-2xl mt-4 sm:mt-10">
                  ...
                  <form onSubmit={async (e) => {
                     ...
                     <input type="text" autoFocus value={identAtiva} onChange={e => setIdentAtiva(e.target.value)} required 
                         className="w-full bg-[#E5E7EB] border-none text-center text-3xl sm:text-5xl p-4 sm:p-6 font-normal text-fg-soft outline-none mb-6 sm:mb-10 tracking-wider" />
               ...
               <div className="bg-slate-800 text-white p-5 flex justify-between items-center shrink-0">
                  <div className="flex items-center gap-3">
                     ...
                     <div>
                        ...
                        <p className="text-xs text-dim font-medium">Adicione ou remova mesas físicas do seu salão</p>
                     </div>
                  </div>
                  <button onClick={() => setModalGestaoMesas(false)} className="text-subtle hover:text-white"><X size={24} /></button>
               </div>
      ...
      {painelOnline && (
        <div className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm">
           <div className="w-full sm:w-[400px] bg-card h-full shadow-2xl flex flex-col animate-slide-in-right">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-slate-50 flex flex-col gap-4 relative">
               {carregandoPainel ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-subtle gap-2 opacity-60">
                     ...
                     <div key={pedido.id} className="bg-card rounded-xl border border-line shadow-sm overflow-hidden flex flex-col">
                        ...
                        <div className="p-3 border-b border-line-soft flex justify-between items-start bg-slate-50/50">
                           ...
                           <div className="text-right">
                              ...
                              <p className="text-3xs text-subtle mt-2">{new Date(pedido.created_at).toLocaleTimeString().slice(0,5)}</p>
                           </div>
                        </div>

                        {/* Itens */}
                        <div className="p-3 bg-card text-xs space-y-2">
                           {pedido.pedidos_itens?.map((it, idx) => (
                              <div key={idx} className="flex justify-between border-b border-slate-50 pb-2 last:border-0 last:pb-0">
                                 <div>
                                    ...
                                    {it.observacao && <p className="text-3xs text-subtle mt-0.5 italic">Obs: {it.observacao}</p>}
                                 </div>
                        ...
                        {pedido.status === 'novo_online' && (
                           <div className="p-3 flex gap-2 border-t border-line bg-card">
