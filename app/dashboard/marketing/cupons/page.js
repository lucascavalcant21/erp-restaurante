    ...
    <div className="p-4 sm:p-8 max-w-6xl mx-auto animate-in fade-in">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted font-bold mt-1">Crie promoções e atraia mais clientes</p>
        </div>
      ...
      {cupons.length === 0 ? (
        <div className="bg-card rounded-[32px] p-16 text-center shadow-sm border border-line flex flex-col items-center justify-center">
          <Tag size={48} className="text-dim mb-4" />
          <h3 className="text-xl font-black text-fg-soft">Nenhum cupom cadastrado</h3>
          <p className="text-muted mt-2">Crie seu primeiro cupom para começar a dar descontos aos clientes.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cupons.map(cupom => {
            ...
            return (
              <div key={cupom.id} className={`bg-card rounded-2xl p-6 border-2 transition-all shadow-sm ${!cupom.ativo || expirado ? 'border-line opacity-60' : 'border-emerald-100 hover:border-emerald-300'} relative group`}>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${!cupom.ativo ? 'bg-elevated text-muted' : expirado ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700'}`}>
                ...
                <div className="flex items-end gap-2 mb-4">
                   ...
                   <span className="text-muted font-bold mb-1">OFF</span>
                </div>

                {cupom.data_validade && (
                   <p className="text-xs font-bold text-subtle flex items-center gap-1">
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card rounded-2xl sm:rounded-[32px] p-4 sm:p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 max-h-[94vh] overflow-y-auto">
            ...
            <form onSubmit={handleSalvar} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-muted uppercase mb-2">Código (Ex: BLACK10)</label>
                <input type="text" required value={formCodigo} onChange={e => setFormCodigo(e.target.value.toUpperCase())} className="w-full bg-slate-50 border border-line rounded-xl px-4 py-3 font-black text-fg-soft uppercase outline-none focus:border-[#4970AF]" placeholder="CÓDIGO" />
              </div>
              
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                   <label className="block text-xs font-bold text-muted uppercase mb-2">Tipo de Desconto</label>
                   <select value={formTipo} onChange={e => setFormTipo(e.target.value)} className="w-full bg-slate-50 border border-line rounded-xl px-4 py-3 font-bold text-fg-soft outline-none focus:border-[#4970AF]">
                      <option value="percentual">Porcentagem (%)</option>
                      <option value="fixo">Valor Fixo (R$)</option>
                   </select>
                </div>
                <div>
                   <label className="block text-xs font-bold text-muted uppercase mb-2">Valor ({formTipo === 'percentual' ? '%' : 'R$'})</label>
                   <input type="number" step="0.01" required value={formValor} onChange={e => setFormValor(e.target.value)} className="w-full bg-slate-50 border border-line rounded-xl px-4 py-3 font-black text-fg-soft outline-none focus:border-[#4970AF]" placeholder="0.00" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-muted uppercase mb-2">Validade (Opcional)</label>
                <input type="date" value={formValidade} onChange={e => setFormValidade(e.target.value)} className="w-full bg-slate-50 border border-line rounded-xl px-4 py-3 font-bold text-fg-soft outline-none focus:border-[#4970AF]" />
              </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                 ...
                 <label htmlFor="ativoCupom" className="font-bold text-fg-soft cursor-pointer">Cupom Ativo</label>
              </div>
