                ...
                return (
                  <Card key={p.id} style={p.ativo ? {} : { opacity: 0.6 }}>
                    ...
                    <div className="flex gap-2" style={{ borderTop: "1px solid var(--line)", paddingTop: 10 }}>
                      <button onClick={podeEditar ? () => toggle(p) : undefined} className="flex-1 flex items-center justify-center gap-1 py-2 text-[11px] font-bold rounded-lg" style={{ background: "var(--elevated)", color: "var(--muted)", opacity: podeEditar ? 1 : 0.5, cursor: podeEditar ? "pointer" : "default" }}>
