            ...
            <div className="relative">
              ...
              {menuPortalAberto && (
                <div className="absolute right-0 top-11 z-20 w-48 rounded-xl border border-line bg-card p-1.5 shadow-xl text-xs font-bold">
                  ...
                  <button onClick={() => { setMenuPortalAberto(false); copiarLinkPortal(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-fg-soft hover:bg-slate-50">{linkCopiado ? <Check size={14} className="text-emerald-600"/> : <Copy size={14}/>} {linkCopiado ? "Link copiado!" : "Copiar link"}</button>
      ...
      <main className="mx-auto max-w-7xl space-y-4 px-4 py-4 sm:px-6">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_300px]">
          <section className="space-y-3">
            <div className="relative">
              <Search className="absolute left-3.5 top-3 text-subtle" size={18} />
              <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar por nome, função ou telefone..." className="h-10 w-full rounded-xl border border-line bg-card pl-10 pr-4 text-sm font-semibold outline-none focus:border-emerald-500 shadow-sm" />
            </div>
            ...
            {!unidadeAtiva || unidadeAtiva === "todas" ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-card p-8 text-center">
                <UsersRound className="mx-auto text-dim" size={36} />
                <p className="mt-2 font-black text-fg-soft">Nenhum extra encontrado</p>
              </div>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2">
                {filtrados.map((extra) => {
                  ...
                  return (
                    <article key={extra.id} className="rounded-3xl border border-line bg-card p-4 shadow-sm transition hover:border-emerald-300 hover:shadow-md flex flex-col justify-between">
                      <div>
                        {/* CABEÇALHO DO CARD */}
                        ...
                        <div className="space-y-1.5 border-t border-line-soft pt-2.5 text-xs font-semibold text-slate-600">
                          <div className="flex items-center justify-between">
                            <span className="flex items-center gap-1.5 text-muted font-medium"><Phone size={13} className="text-emerald-600 shrink-0" /> {fmtTel(extra.telefone)}</span>
                            <span className="font-black text-fg">Diária: <strong className="text-emerald-700 font-black">{fmtBRL(extra.salario)}</strong></span>
                          </div>
                          ...
                          <div className="mt-2.5 rounded-2xl bg-slate-50/80 p-2.5 text-xs border border-line-soft grid grid-cols-3 gap-2 text-center">
                            ...
                            <div>
                              <span className="text-3xs font-bold text-subtle block uppercase tracking-wider">Trabalhos</span>
                              <strong className="text-slate-800 font-black text-sm">{diasTrabalhadosCount} {diasTrabalhadosCount === 1 ? "dia" : "dias"}</strong>
                            </div>
                            <div className="min-w-0">
                              <span className="text-3xs font-bold text-subtle block uppercase tracking-wider">Função</span>
                              <strong className="text-fg-soft font-bold text-xs truncate block" title={funcoesTexto}>{funcoesTexto}</strong>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <aside className="h-fit rounded-2xl border border-line bg-card p-3.5 shadow-sm lg:sticky lg:top-4">
            <div className="flex items-center gap-2 mb-3">
              <FileClock className="text-emerald-600" size={18} />
              <h2 className="text-sm font-black text-fg">Recibos recentes</h2>
            </div>
      ...
      {historicoModal && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => setHistoricoModal(null)}>
            <div className="bg-card rounded-3xl w-full max-w-2xl max-h-[90vh] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
                  ...
                  {recibosPessoa.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {recibosPessoa.map(r => {
                        ...
                        return (
                          <div key={r.id} className="rounded-2xl border border-line p-3.5 bg-card hover:border-emerald-300 transition-all flex flex-wrap items-center justify-between gap-3">
                            <div>
      ...
      {reciboAcaoModal && (() => {
        ...
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" onClick={() => setReciboAcaoModal(null)}>
            <div className="bg-card rounded-3xl w-full max-w-md shadow-2xl p-6 overflow-hidden animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
              <div className="flex items-start justify-between gap-3 mb-4 border-b border-line-soft pb-3">
                <div>
