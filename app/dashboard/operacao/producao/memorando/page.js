...
function SetorPlanejamento({ titulo, icone: Icone, cor, fichas, plano, colaboradores, alterar }) {
  return <section className="overflow-hidden rounded-3xl border border-line bg-card shadow-sm">
    ...
    <div className="space-y-2 p-3 sm:p-5">
      {fichas.length === 0 ? <p className="p-5 text-center text-sm font-bold text-subtle">Nenhuma ficha cadastrada neste setor.</p> : fichas.map(ficha => {
        const item = plano[ficha.id] || {};
        const ativo = numero(item.qtd) > 0;
        return <article key={ficha.id} className={`rounded-2xl border p-3 transition ${ativo ? "border-emerald-300 bg-emerald-50/40" : "border-line bg-slate-50/50"}`}>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <strong className="min-w-0 flex-1 text-sm text-slate-800">{ficha.nome_receita}</strong>
            <div className="flex items-center gap-2">
              <input type="number" min="0" step="0.01" inputMode="decimal" value={item.qtd || ""} onChange={e => alterar(ficha.id, { qtd: e.target.value })} placeholder="0" className="h-10 w-24 rounded-xl border border-line bg-card px-3 text-center font-black outline-none focus:border-emerald-500"/>
              <span className="text-3xs font-bold uppercase text-subtle">porções</span>
            </div>
          </div>
          {ativo && <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <select value={item.responsavel || ""} onChange={e => alterar(ficha.id, { responsavel: e.target.value })} className="h-10 rounded-xl border border-line bg-card px-3 text-xs font-bold text-slate-600 outline-none">
            ...
            <input value={item.observacao || ""} onChange={e => alterar(ficha.id, { observacao: e.target.value })} placeholder="Observação: prioridade, armazenamento..." className="h-10 rounded-xl border border-line bg-card px-3 text-xs font-semibold outline-none"/>
          </div>}
        </article>;
      })}
    </div>
...
export default function MemorandoProducaoPage() {
  ...
  const [setorAtivo, setSetorAtivo] = useState("cozinha");
  ...
  const nomeSetorAtivo = setorAtivo === "bar" ? "Bar" : "Cozinha";
  const planejadosAtivos = useMemo(() => planejados.filter(item => item.setor === nomeSetorAtivo), [nomeSetorAtivo, planejados]);
  const comprasManuaisAtivas = useMemo(() => comprasManuais.filter(item => (item.setor || "cozinha") === setorAtivo), [comprasManuais, setorAtivo]);

  const comprasAutomaticas = useMemo(() => {
    const consumo = new Map();
    planejadosAtivos.forEach(item => {
  ...
  }, [estoque, planejadosAtivos, todasFichas]);
  ...
  const adicionarCompraManual = () => {
    ...
    setComprasManuais(atual => [...atual, { ...novaCompra, setor: setorAtivo, id: `manual:${Date.now()}`, nome: novaCompra.nome.trim() }]);
  ...
  const enviarProducaoWhatsApp = () => {
    if (!planejadosAtivos.length) return setAviso(`Nenhuma produção do ${nomeSetorAtivo} planejada para esta data.`);
    const dataFmt = dataReferencia.split("-").reverse().join("/");
    let txt = `*PRODUÇÃO DO ${nomeSetorAtivo.toUpperCase()}*\n${unidadeInfo?.nome || "Restaurante"}\n${dataFmt}\n`;
    txt += `----------------------------------\n`;
      planejadosAtivos.forEach(item => {
    ...
    txt += `\n${planejadosAtivos.length} preparação(ões) do ${nomeSetorAtivo}.`;
  ...
  const enviarComprasWhatsApp = () => {
    const compras = [
      ...
      ...comprasManuaisAtivas.map(item => ({ nome: item.nome, quantidade: item.quantidade, unidade: item.unidade, detalhe: item.observacao || "" })),
    ];
  ...
  const imprimir = () => {
    if (!planejadosAtivos.length && !comprasAutomaticas.length && !comprasManuaisAtivas.length) return setAviso(`O memorando do ${nomeSetorAtivo} está vazio.`);
    const dataFmt = dataReferencia.split("-").reverse().join("/");
  ...
  return <div className="min-h-screen bg-[var(--surface)] pb-24 text-slate-800">
    <header className="sticky top-0 z-20 border-b border-line bg-white/95 px-4 py-4 backdrop-blur sm:px-6">
      <div className="mx-auto flex max-w-7xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3"><button onClick={() => router.back()} className="grid h-11 w-11 place-items-center rounded-xl border border-line bg-slate-50 text-slate-600"><ArrowLeft size={20}/></button><div><h1 className="text-2xl font-black tracking-tight">Produção do {nomeSetorAtivo}</h1><p className="text-xs font-bold text-muted">Lista separada · {unidadeInfo?.nome}</p></div></div>
        <div className="flex flex-wrap items-center gap-2"><input type="date" value={dataReferencia} onChange={e => setDataReferencia(e.target.value)} className="h-11 rounded-xl border border-line bg-card px-3 font-bold"/><select value={status} onChange={e => setStatus(e.target.value)} className="h-11 rounded-xl border border-line bg-card px-3 font-bold"><option value="rascunho">Rascunho</option><option value="confirmado">Confirmado</option></select><button onClick={salvar} disabled={salvando} className="flex h-11 items-center gap-2 rounded-xl bg-accent px-4 font-black text-accent-fg disabled:opacity-50"><Save size={18}/>{salvando ? "Salvando..." : "Salvar"}</button><button onClick={enviarProducaoWhatsApp} className="flex h-11 items-center gap-2 rounded-xl border-2 border-emerald-200 bg-card px-4 font-black text-accent-strong hover:bg-accent-soft"><Share2 size={18}/>Produção no WhatsApp</button><button onClick={enviarComprasWhatsApp} className="flex h-11 items-center gap-2 rounded-xl border-2 border-emerald-200 bg-card px-4 font-black text-accent-strong hover:bg-accent-soft"><Share2 size={18}/>Compras no WhatsApp</button><button onClick={imprimir} className="flex h-11 items-center gap-2 rounded-xl bg-slate-900 px-4 font-black text-white"><Printer size={18}/>Imprimir</button></div>
      </div>
    </header>

    <main className="mx-auto max-w-7xl space-y-5 px-4 py-6 sm:px-6">
      <div className="grid grid-cols-2 gap-2 rounded-2xl bg-slate-200 p-1.5"><button onClick={() => setSetorAtivo("cozinha")} className={`flex min-h-12 items-center justify-center gap-2 rounded-xl font-black ${setorAtivo === "cozinha" ? "bg-accent text-accent-fg shadow" : "text-slate-600"}`}><ChefHat size={19}/>Cozinha</button><button onClick={() => setSetorAtivo("bar")} className={`flex min-h-12 items-center justify-center gap-2 rounded-xl font-black ${setorAtivo === "bar" ? "bg-blue-700 text-white shadow" : "text-slate-600"}`}><GlassWater size={19}/>Bar</button></div>
      {aviso && <div className="flex items-center justify-between rounded-2xl border border-emerald-200 bg-accent-soft px-4 py-3 text-sm font-bold text-accent-strong"><span>{aviso}</span><button onClick={() => setAviso("")} className="text-success">×</button></div>}
      {loading ? <div className="p-16 text-center font-bold text-subtle">Carregando fichas e estoque...</div> : setorAtivo === "cozinha" ? <SetorPlanejamento titulo="Produção da Cozinha" icone={ChefHat} cor="bg-emerald-700" fichas={fichasCozinha} plano={cozinha} colaboradores={colaboradores} alterar={(id, patch) => alterarPlano(setCozinha, id, patch)}/> : <SetorPlanejamento titulo="Produção do Bar" icone={GlassWater} cor="bg-blue-700" fichas={fichasBar} plano={bar} colaboradores={colaboradores} alterar={(id, patch) => alterarPlano(setBar, id, patch)}/>}
      ...
      <section className="overflow-hidden rounded-3xl border border-line bg-card shadow-sm">
        ...
        <div className="p-4 sm:p-5">
          {comprasAutomaticas.length > 0 && <div className="mb-4 overflow-x-auto"><table className="w-full min-w-[620px] text-left text-xs"><thead className="bg-elevated text-3xs font-bold uppercase text-muted"><tr><th className="p-3">Item</th><th className="p-3">Necessário</th><th className="p-3">Saldo</th><th className="p-3">Comprar</th></tr></thead><tbody className="divide-y divide-slate-100">{comprasAutomaticas.map(item => <tr key={item.id}><td className="p-3 font-black">{item.nome}</td><td className="p-3">{item.necessario.toLocaleString("pt-BR", { maximumFractionDigits: 3 })} {item.unidade}</td><td className="p-3">{item.disponivel.toLocaleString("pt-BR", { maximumFractionDigits: 3 })} {item.unidade}</td><td className="p-3 font-black text-rose-600">{item.comprar.toLocaleString("pt-BR", { maximumFractionDigits: 3 })} {item.unidade}</td></tr>)}</tbody></table></div>}
          {comprasAutomaticas.length === 0 && <p className="mb-4 rounded-xl bg-accent-soft p-3 text-sm font-bold text-accent-strong">O estoque atual cobre as produções planejadas.</p>}
          <div className="grid gap-2 rounded-2xl border border-dashed border-amber-300 bg-amber-50/50 p-3 sm:grid-cols-[1fr_110px_90px_1fr_auto]"><input value={novaCompra.nome} onChange={e => setNovaCompra({ ...novaCompra, nome: e.target.value })} placeholder="Adicionar compra manual" className="h-10 rounded-xl border border-line bg-card px-3 text-sm font-bold outline-none"/><input value={novaCompra.quantidade} onChange={e => setNovaCompra({ ...novaCompra, quantidade: e.target.value })} inputMode="decimal" placeholder="Qtd" className="h-10 rounded-xl border border-line bg-card px-3 text-sm font-bold outline-none"/><select value={novaCompra.unidade} onChange={e => setNovaCompra({ ...novaCompra, unidade: e.target.value })} className="h-10 rounded-xl border border-line bg-card px-2 text-xs font-bold"><option>UN</option><option>KG</option><option>G</option><option>L</option><option>ML</option><option>CX</option><option>PCT</option></select><input value={novaCompra.observacao} onChange={e => setNovaCompra({ ...novaCompra, observacao: e.target.value })} placeholder="Observação/fornecedor" className="h-10 rounded-xl border border-line bg-card px-3 text-sm font-semibold outline-none"/><button onClick={adicionarCompraManual} className="flex h-10 items-center justify-center gap-1 rounded-xl bg-amber-500 px-3 text-xs font-bold text-white"><Plus size={16}/>Adicionar</button></div>
          {comprasManuaisAtivas.length > 0 && <div className="mt-3 grid gap-2 sm:grid-cols-2">{comprasManuaisAtivas.map(item => <article key={item.id} className="flex items-center gap-3 rounded-xl border border-line p-3"><div className="min-w-0 flex-1"><strong className="block truncate text-sm">{item.nome}</strong><small className="text-muted">{item.quantidade} {item.unidade}{item.observacao ? ` · ${item.observacao}` : ""}</small></div><button onClick={() => setComprasManuais(atual => atual.filter(compra => compra.id !== item.id))} className="grid h-9 w-9 place-items-center rounded-lg bg-rose-50 text-rose-600"><Trash2 size={16}/></button></article>)}</div>}
        </div>
      </section>

      <section className="rounded-3xl border border-line bg-card p-5"><label className="mb-2 flex items-center gap-2 text-sm font-black text-fg-soft"><ClipboardList size={18}/>Observações gerais do memorando</label><textarea rows={4} value={observacoes} onChange={e => setObservacoes(e.target.value)} placeholder="Prioridades, entregas, eventos, armazenamento e recados para as equipes..." className="w-full rounded-2xl border border-line bg-slate-50 p-4 text-sm font-semibold outline-none focus:border-emerald-500"/><div className="mt-4 flex items-center justify-between text-xs font-bold text-muted"><span>{planejadosAtivos.length} produção(ões) · {comprasAutomaticas.length + comprasManuaisAtivas.length} compra(s) do {nomeSetorAtivo}</span>{status === "confirmado" && <span className="flex items-center gap-1 text-success"><CheckCircle2 size={16}/>Memorando confirmado</span>}</div></section>
    </main>
  </div>;
}
