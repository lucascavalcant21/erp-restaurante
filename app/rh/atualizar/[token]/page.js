...
const INPUT = "mt-1.5 h-11 w-full rounded-xl border border-line bg-card px-3.5 text-sm font-semibold text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100";

function Campo({ label, children, largo = false }) {
  return <label className={largo ? "sm:col-span-2" : ""}><span className="text-xs font-bold text-slate-600">{label} <b className="text-rose-500">*</b></span>{children}</label>;
}
...
export default function AtualizarPerfilRH() {
  ...
  if (carregando) return <main className="min-h-screen bg-slate-50 grid place-items-center"><div className="text-center"><Loader2 className="mx-auto animate-spin text-success"/><p className="mt-3 font-bold text-muted">Abrindo seu cadastro...</p></div></main>;
  if (erro && !perfil) return <main className="min-h-screen bg-slate-50 grid place-items-center p-5"><div className="max-w-md rounded-3xl border border-rose-200 bg-card p-8 text-center shadow-xl"><ShieldCheck className="mx-auto text-rose-500" size={42}/><h1 className="mt-4 text-xl font-black text-fg">Este link não está disponível</h1><p className="mt-2 text-sm font-medium text-muted">{erro}</p><p className="mt-4 text-xs text-subtle">Peça ao RH um novo link de atualização.</p></div></main>;

  return <main className="min-h-screen bg-elevated px-3 py-6 sm:px-6 sm:py-10"><form onSubmit={salvar} className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-line bg-card shadow-xl">
    <header className="bg-gradient-to-br from-emerald-800 to-emerald-600 px-5 py-7 text-white sm:px-8"><div className="flex items-center gap-4"><span className="grid h-14 w-14 place-items-center rounded-2xl bg-white/15"><UserRound size={30}/></span><div><p className="text-xs font-bold uppercase tracking-[.18em] text-emerald-100">Atualização segura do RH</p><h1 className="mt-1 text-2xl font-black">Olá, {perfil.nome?.split(" ")[0] || "colaborador"}</h1><p className="mt-1 text-sm text-emerald-50">Confira seus dados e corrija o que for necessário.</p></div></div></header>
    <div className="space-y-7 p-5 sm:p-8">
      {salvo && <div className="flex items-start gap-3 rounded-2xl border border-emerald-200 bg-accent-soft p-4 text-accent-strong"><CheckCircle2 className="shrink-0"/><div><p className="font-black">Dados atualizados com sucesso</p><p className="text-sm">O RH já consegue ver as novas informações.</p></div></div>}
      {erro && <p className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm font-bold text-rose-700">{erro}</p>}
      <section><h2 className="mb-4 text-sm font-black uppercase tracking-wider text-accent">Dados pessoais</h2><div className="grid gap-4 sm:grid-cols-2">
      ...
      <section><h2 className="mb-4 text-sm font-black uppercase tracking-wider text-accent">Endereço</h2><div className="grid gap-4 sm:grid-cols-2">
      ...
      <section><h2 className="mb-4 text-sm font-black uppercase tracking-wider text-accent">Família, pagamento e transporte</h2><div className="grid gap-4 sm:grid-cols-2"><Campo label="Nome da mãe"><input required value={perfil.nome_mae || ""} onChange={e=>set("nome_mae",e.target.value)} className={INPUT}/></Campo><Campo label="Nome do pai"><input required value={perfil.nome_pai || ""} onChange={e=>set("nome_pai",e.target.value)} className={INPUT}/></Campo><Campo label="Chave Pix" largo><input required value={perfil.chave_pix || ""} onChange={e=>set("chave_pix",e.target.value)} className={INPUT}/></Campo>
      ...
      </div>{perfil.tem_filhos && <div className="mt-5"><div className="mb-2 flex items-center justify-between"><p className="text-xs font-bold text-slate-600">Filhos / dependentes <b className="text-rose-500">*</b></p><button type="button" onClick={()=>set("filhos",[...(perfil.filhos||[]),{nome:"",data_nascimento:""}])} className="flex items-center gap-1 text-xs font-bold text-accent"><Plus size={14}/> Adicionar</button></div><div className="space-y-2">{(perfil.filhos||[]).map((filho,i)=><div key={i} className="grid grid-cols-[1fr_140px_36px] gap-2"><input required value={filho.nome||""} onChange={e=>set("filhos",perfil.filhos.map((x,j)=>j===i?{...x,nome:e.target.value}:x))} placeholder="Nome" className={INPUT.replace("mt-1.5 ","")}/><input required type="date" value={String(filho.data_nascimento||"").slice(0,10)} onChange={e=>set("filhos",perfil.filhos.map((x,j)=>j===i?{...x,data_nascimento:e.target.value}:x))} className={INPUT.replace("mt-1.5 ","")}/><button type="button" onClick={()=>set("filhos",perfil.filhos.filter((_,j)=>j!==i))} className="grid place-items-center rounded-xl bg-rose-50 text-rose-600"><Trash2 size={16}/></button></div>)}</div></div>}</section>
      <p className="flex items-start gap-2 rounded-xl bg-slate-50 p-3 text-xs font-medium text-muted"><ShieldCheck size={16} className="shrink-0 text-success"/>Todos os campos com * são obrigatórios. Este formulário não mostra salário, avaliações ou histórico. O link é pessoal e vence automaticamente.</p>
    </div><footer className="sticky bottom-0 border-t border-line bg-white/95 p-4 backdrop-blur sm:px-8"><button disabled={salvando} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-accent font-black text-accent-fg shadow-lg shadow-emerald-700/20 disabled:opacity-60">{salvando?<Loader2 className="animate-spin" size={19}/>:<Save size={19}/>} {salvando?"Salvando...":"Salvar meus dados"}</button></footer>
  </form></main>;
}
