...
import { Settings, Briefcase, Clock, DollarSign, Edit3, Trash2, Info } from "lucide-react";
import {
...
import { fetchCargos, inserirCargo, atualizarCargo, removerCargo, fetchTurnos, inserirTurno, atualizarTurno, removerTurno, inserirCargosPadrao } from "../../../lib/rh";
  ...
  return (
    <div className="min-h-screen">
      <PageHeader title="Configurações de RH" subtitle={`Ajustes do Módulo · ${unidadeInfo.nome}`} icon={Settings}
        onAction={() => { setEditar(null); setModal(true); }} actionLabel={`Novo ${atual.title}`} />
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editar ? `Editar ${atual.title}` : `Novo ${atual.title}`}>
