...
const CATEGORIA_STYLE = {
  ...
  neutro:   { label: "Neutro",    Icon: Minus,       bg: "bg-[#334155]", cor: "text-[#64748B]" },
  detrator: { label: "Detrator",  Icon: ThumbsDown,  bg: "bg-[rgba(5,150,105,0.15)]",    cor: "text-[#059669]"    },
};
...
function BotaoNota({ val, selecionado, onClick }) {
  const cor = val >= 9 ? "border-emerald-400 bg-emerald-50 text-emerald-700"
            : val >= 7 ? "border-white/10  text-[#94A3B8]"
            :             "border-[rgba(5,150,105,0.4)] bg-[rgba(5,150,105,0.1)] text-[#059669]";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-[#059669]/40" onClick={onFechar}>
      <div className="w-full bg-[#1E293B] rounded-t-3xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-lg font-black text-[#F1F5F9]">Nova Avaliação</h2>

        <div>
          <label className="text-[11px] font-black text-[#475569] uppercase tracking-wider block mb-1">Nome (opcional)</label>
        ...
        <div>
          <label className="text-[11px] font-black text-[#475569] uppercase tracking-wider block mb-2">Unidade</label>
          <div className="flex gap-2">
            {["Seldeestrela", "Tico Tico Saladas", "Burguer"].map(u => (
              <button key={u} onClick={() => setUnidade(u)}
                className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all active:scale-95 ${unidade === u ? "bg-[#059669] text-white" : "bg-[#334155] text-[#94A3B8]"}`}>
                {u.split(" ")[0]}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="text-[11px] font-black text-[#475569] uppercase tracking-wider block mb-2">Nota (0–10)</label>
          ...
          {nota !== null && (
            <p className={`text-[11px] font-black mt-2 ${nota >= 9 ? "text-emerald-700" : nota >= 7 ? "text-[#94A3B8]" : "text-[#059669]"}`}>
        ...
        <div>
          <label className="text-[11px] font-black text-[#475569] uppercase tracking-wider block mb-1">Comentário</label>
        ...
        <button
          ...
          disabled={nota === null}
          className="w-full py-3.5 rounded-2xl bg-[#059669] text-white font-black text-sm active:scale-95 transition-transform disabled:opacity-40">
          Enviar Avaliação
        </button>
...
function CardAvaliacao({ aval }) {
  ...
  const Icon = cat.Icon;
  return (
    <div className="bg-[#1E293B] rounded-2xl border border-white/5  px-4 py-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-0.5">
            <p className="text-[12px] font-black text-[#F1F5F9]">{aval.nome}</p>
            <div className="flex items-center gap-1">
              <span className="text-[11px] font-black text-[#F1F5F9]">{aval.nota}</span>
          ...
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-[10px] text-[#475569]">{aval.unidade}</span>
            <span className="text-[10px] text-[#475569]">{new Date(aval.data + "T12:00:00").toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })}</span>
          </div>
          {aval.comentario && <p className="text-[11px] font-medium text-[#94A3B8] leading-snug">{aval.comentario}</p>}
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen ">
      <div className="sticky top-0 z-20  border-b border-white/8 px-4 pt-12 pb-3" style={{ background: '#0F172A' }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-[#475569] font-medium">Satisfação dos clientes</p>
          </div>
      ...
      <div className="px-4 pt-4 pb-28 space-y-4">
        ...
        <div className={`rounded-2xl border p-5 ${st.bg} ${st.borda}`}>
          <p className="text-[10px] font-black text-[#64748B] uppercase tracking-wider mb-1">NPS Score</p>
          <div className="flex items-end gap-3 mb-3">
            <p className={`text-6xl font-black leading-none ${st.cor}`}>{nps.score}</p>
            <div className="pb-1">
              <p className={`text-sm font-black ${st.cor}`}>{st.label}</p>
              <p className="text-[11px] text-[#64748B] font-medium">{avaliacoes.length} avaliações</p>
            </div>
          </div>
          {/* Barra NPS */}
          ...
          <div className="flex justify-between text-[10px] font-black">
            <span className="text-[#059669]">🔴 Detratores {nps.pct_d}%</span>
            <span className="text-[#64748B]">⬜ Neutros {nps.pct_n}%</span>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-[#1E293B] border border-white/5 rounded-2xl  p-3 text-center">
            <p className="text-[9px] font-black text-[#475569] uppercase tracking-wider mb-1">Neutros</p>
            <p className="text-2xl font-black text-[#F1F5F9]">{nps.neutros}</p>
          </div>
          <div className={`rounded-2xl border p-3 text-center ${nps.detratores > 0 ? "bg-[rgba(5,150,105,0.1)] border-[rgba(5,150,105,0.3)]" : "bg-[#1E293B] border-white/5"}`}>
            <p className={`text-[9px] font-black uppercase tracking-wider mb-1 ${nps.detratores > 0 ? "text-[#10b981]" : "text-[#475569]"}`}>Detratores</p>
            <p className={`text-2xl font-black ${nps.detratores > 0 ? "text-[#059669]" : "text-[#F1F5F9]"}`}>{nps.detratores}</p>
          </div>
        </div>

        {/* Histograma de notas */}
        <div className="bg-[#1E293B] rounded-2xl border border-white/5  p-4">
          <p className="text-[10px] font-black text-[#475569] uppercase tracking-wider mb-3">Distribuição de Notas</p>
          <div className="flex items-end gap-1 h-16">
            {histograma.map(({ nota, count, pct }) => (
              <div key={nota} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="text-[8px] font-black text-[#64748B]">{nota}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          {UNIDADES_OPT.map(u => (
            <button key={u} onClick={() => setUnidade(u)}
              className={`flex-shrink-0 text-[11px] font-black px-3 py-1.5 rounded-full transition-all active:scale-95 ${unidade === u ? "bg-emerald-700 text-white" : "bg-[#1E293B] text-[#94A3B8] border border-white/8"}`}>
              {u.split(" ")[0]}
            </button>
          ))}
        </div>
        ...
        <div className="flex justify-between px-1">
          <p className="text-[11px] font-black text-[#475569] uppercase tracking-wider">Avaliações</p>
          <p className="text-[11px] font-bold text-[#475569]">{filtradas.length}</p>
        </div>
