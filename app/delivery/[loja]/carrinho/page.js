...
export default function CarrinhoPage({ params }) {
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white flex flex-col items-center justify-center p-4 sm:p-6 text-center">
        ...
        <div className="bg-white/10 p-6 rounded-2xl border border-white/10 w-full max-w-sm mb-6">
          ...
          <p className="text-xs text-dim">Esses pontos de fogo já estão na sua carteira digital.</p>
        </div>

        {pagamentoInfo?.metodo === "PIX" && (
          <div className="bg-card text-fg p-6 rounded-2xl w-full max-w-sm space-y-4">
            <h2 className="font-bold">Pagamento Inteligente (PagSeguro)</h2>
            <div className="p-3 bg-elevated rounded-lg text-xs break-all border border-slate-300 font-mono">
            ...
            <p className="text-xs text-muted mt-2">Pague agora para liberar a entrega.</p>
          </div>
        )}
        ...
        <button onClick={() => router.push(`/delivery/${lojaSlug}`)} className="mt-8 text-sm text-subtle underline">
          Voltar para a loja
        </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-fg flex flex-col">
      <header className="bg-card px-4 py-4 flex items-center gap-4 sticky top-0 z-20 shadow-sm border-b border-line">
        <button onClick={() => etapa === "checkout" ? setEtapa("sacola") : router.back()} className="w-10 h-10 bg-elevated rounded-full flex items-center justify-center text-slate-600 active:scale-95">
          <ArrowLeft size={20} />
        </button>
      ...
      <main className="flex-1 p-4 pb-32 max-w-md mx-auto w-full">
        {etapa === "sacola" && (
          <div className="space-y-4">
            {carrinho.map(c => (
              <div key={c.id} className="bg-card p-4 rounded-2xl shadow-sm border border-line-soft flex gap-4">
                <div className="flex-1">
                  <h3 className="font-bold text-fg">{c.qtd}x {c.nome}</h3>
            ...
            <div className="bg-card p-4 rounded-2xl shadow-sm border border-line-soft space-y-2 mt-6">
        ...
        {etapa === "checkout" && (
          <div className="space-y-6 animate-in slide-in-from-right duration-300">
            {/* Dados do Cliente */}
            <div className="bg-card p-5 rounded-3xl shadow-sm border border-line-soft space-y-4">
            ...
            <div className="bg-card p-5 rounded-3xl shadow-sm border border-line-soft space-y-4">
              ...
              <input type="text" placeholder="CEP (Apenas números)" maxLength={8} className="w-full bg-slate-50 border border-line p-3 rounded-xl outline-none focus:border-slate-400 font-bold tracking-widest text-center"
                value={form.cep} onChange={e => setForm({...form, cep: e.target.value})} />
              
              {loadingTaxa && <p className="text-sm text-muted text-center animate-pulse">Calculando distância (Google Maps)...</p>}
              
              {form.endereco && (
                <div className="p-3 bg-elevated rounded-xl text-sm">
                  <p className="font-medium text-fg-soft">{form.endereco}</p>
                  <input type="text" placeholder="Número e Complemento" className="w-full mt-2 bg-card border border-line p-2 rounded-lg outline-none text-sm"
            ...
            <div className="bg-card p-5 rounded-3xl shadow-sm border border-line-soft space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => setForm({...form, formaPagamento: "PIX"})} className={`flex-1 py-3 rounded-xl font-bold border-2 transition ${form.formaPagamento === "PIX" ? 'border-teal-500 bg-teal-50 text-teal-700' : 'border-line-soft text-subtle'}`}>PIX</button>
                <button onClick={() => setForm({...form, formaPagamento: "CARTAO"})} className={`flex-1 py-3 rounded-xl font-bold border-2 transition ${form.formaPagamento === "CARTAO" ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-line-soft text-subtle'}`}>CARTÃO</button>
              </div>
            </div>
      ...
      {/* FOOTER TOTAL */}
      <div className="fixed bottom-0 left-0 right-0 bg-card border-t border-line p-4 px-4 sm:px-6 z-30 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]" style={{ paddingBottom: "max(1rem, env(safe-area-inset-bottom))" }}>
        <div className="flex justify-between items-end mb-4">
          <span className="text-muted font-medium">Total a Pagar</span>
          <span className="text-2xl font-bold text-fg">R$ {totalGeral.toFixed(2).replace('.', ',')}</span>
        </div>
