...
function FormFuncionario({ inicial, onSalvar, onCancelar }) {
  ...
  const sel = "w-full appearance-none  border border-white/8 rounded-xl px-3 py-3.5 text-sm font-bold text-[#F1F5F9] focus:outline-none focus:ring-2 focus:border-[#10b981] pr-8";

  return (
    <div className="bg-[#1E293B] rounded-2xl border border-white/5  p-5 space-y-4">
      <p className="text-sm font-black text-[#F1F5F9]">{inicial ? "Editar Funcionário" : "Novo Funcionário"}</p>

      <div>
        <label className="text-[10px] font-black text-[#475569] uppercase tracking-wider block mb-1.5">Nome Completo *</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-[10px] font-black text-[#475569] uppercase tracking-wider block mb-1.5">Cargo</label>
          <div className="relative"><select value={cargo} onChange={e => setCargo(e.target.value)} className={sel}>
            {CARGOS.map(c => <option key={c} value={c}>{c}</option>)}
          </select><ChevronDown size={13} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#475569] pointer-events-none" /></div>
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-[10px] font-black text-[#475569] uppercase tracking-wider block mb-1.5">Turno</label>
          <div className="relative"><select value={turno} onChange={e => setTurno(e.target.value)} className={sel}>
            {TURNOS.map(t => <option key={t} value={t}>{t}</option>)}
          </select><ChevronDown size={13} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#475569] pointer-events-none" /></div>
        </div>
        <div>
          <label className="text-[10px] font-black text-[#475569] uppercase tracking-wider block mb-1.5">Salário (R$)</label>
      ...
      <div>
        <label className="text-[10px] font-black text-[#475569] uppercase tracking-wider block mb-1.5">Data de Admissão *</label>
...
function CardFuncionario({ f, onEditar, onToggle, onDeletar }) {
  ...
  return (
    <div className={`bg-[#1E293B] rounded-2xl border  overflow-hidden ${!f.ativo ? "opacity-60 border-white/8" : "border-white/5"}`}>
      <button className="w-full px-4 pt-4 pb-3 text-left" onClick={() => setExpandido(e => !e)}>
        <div className="flex items-center gap-3">
          <div className={`w-11 h-11 rounded-2xl flex items-center justify-center text-lg font-black flex-shrink-0 ${f.ativo ? "bg-[#10b981]/10 text-[#10b981]" : "bg-[#334155] text-[#475569]"}`}>
            {f.nome.split(" ").map(n => n[0]).slice(0, 2).join("")}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <p className="text-sm font-black text-[#F1F5F9] truncate">{f.nome}</p>
              {!f.ativo && <span className="text-[9px] font-black bg-[#334155] text-[#64748B] px-1.5 py-0.5 rounded-full uppercase flex-shrink-0">Inativo</span>}
            </div>
            <p className="text-[11px] text-[#64748B] font-medium truncate">{f.cargo} · {f.unidade}</p>
          </div>
          <div className="text-right flex-shrink-0">
            <p className="text-sm font-black text-[#F1F5F9]">{fmtBRL(f.salario)}</p>
            <p className="text-[10px] text-[#475569] font-medium">{tempo} de casa</p>
          </div>
        </div>
      </button>
        ...
        <div className="px-4 pb-4 space-y-2 border-t border-neutral-50 pt-3">
          <div className="grid grid-cols-2 gap-2">
            <div className=" rounded-xl px-3 py-2">
              ...
              <p className="text-xs font-bold text-[#F1F5F9]">{f.turno}</p>
            </div>
            <div className=" rounded-xl px-3 py-2">
              <p className="text-[9px] font-black text-[#475569] uppercase">Admissão</p>
              <p className="text-xs font-bold text-[#F1F5F9]">{fmtData(f.admissao)}</p>
            </div>
          </div>
          {f.telefone && (
      ...
      <div className="flex items-center border-t border-neutral-50 divide-x divide-neutral-50">
        <button onClick={() => onEditar(f)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-black text-[#94A3B8] active: transition-colors">
          <Edit3 size={13} /> Editar
        </button>
        <button onClick={() => onToggle(f.id)} className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-black transition-colors active: ${f.ativo ? "text-[#64748B]" : "text-emerald-600"}`}>
          {f.ativo ? <ToggleRight size={15} /> : <ToggleLeft size={15} />}
          {f.ativo ? "Desativar" : "Reativar"}
        </button>
        <button onClick={() => onDeletar(f.id)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-[11px] font-black text-[#10b981] active:bg-[rgba(5,150,105,0.1)] transition-colors">
    ...
    <div className="min-h-screen ">
      <div className="sticky top-0 z-20  border-b border-white/8 px-4 pt-12 pb-3 flex items-center gap-3" style={{ background: '#0F172A' }}>
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-[#475569] font-medium">Equipe, cargos e folha de pagamento</p>
        </div>
      ...
      <div className="px-4 pt-4 pb-28 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-[#1E293B] rounded-2xl border border-white/5  p-3">
            <div className="w-7 h-7 rounded-lg bg-emerald-100 flex items-center justify-center mb-1.5">
              <UserCheck size={14} className="text-emerald-600" />
            </div>
            <p className="text-xl font-black text-[#F1F5F9]">{resumo.ativos}</p>
            <p className="text-[10px] font-bold text-[#475569]">Ativos</p>
          </div>
          <div className="bg-[#1E293B] rounded-2xl border border-white/5  p-3">
            <div className="w-7 h-7 rounded-lg bg-[#334155] flex items-center justify-center mb-1.5">
              <UserX size={14} className="text-[#475569]" />
            </div>
            <p className="text-xl font-black text-[#F1F5F9]">{resumo.inativos}</p>
            <p className="text-[10px] font-bold text-[#475569]">Inativos</p>
          </div>
          <div className="bg-[#1E293B] rounded-2xl border border-white/5  p-3">
            <div className="w-7 h-7 rounded-lg bg-blue-100 flex items-center justify-center mb-1.5">
              <DollarSign size={14} className="text-blue-600" />
            </div>
            <p className="text-base font-black leading-tight text-[#F1F5F9]">{fmtBRL(resumo.folha)}</p>
            <p className="text-[10px] font-bold text-[#475569]">Folha/mês</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
          {["Todos", ...UNIDADES].map(u => (
            <button key={u} onClick={() => setFiltroUnid(u)}
              className={`flex-shrink-0 text-[11px] font-black px-3 py-1.5 rounded-full transition-all active:scale-95 ${filtroUnid === u ? "bg-[#059669] text-white" : "bg-[#1E293B] text-[#94A3B8] border border-white/8"}`}>
              {u}
            </button>
          ))}
        </div>
        ...
        <div>
          <div className="flex justify-between px-1 mb-2">
            <p className="text-[11px] font-black text-[#475569] uppercase tracking-wider">Equipe</p>
            <p className="text-[11px] font-bold text-[#475569]">{filtrados.length} funcionário{filtrados.length !== 1 ? "s" : ""}</p>
          </div>
          {filtrados.length === 0 ? (
            <div className="bg-[#1E293B] rounded-2xl border border-white/5  p-8 flex flex-col items-center text-center gap-2">
              ...
              <p className="text-sm font-bold text-[#64748B]">Nenhum funcionário encontrado</p>
            </div>
          ) : (
