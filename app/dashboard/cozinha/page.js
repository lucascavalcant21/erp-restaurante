  ...
  return (
    <div className="min-h-screen font-sans pb-24 text-slate-800">
      ...
      <div className="pt-5 sm:pt-6 pb-6 sm:pb-8 px-4 sm:px-6 max-w-6xl mx-auto flex items-center gap-3 sm:gap-4">
         ...
         <div>
            <h1 className="text-3xl sm:text-4xl font-black tracking-tighter text-fg">Op. Cozinha</h1>
            <p className="text-fg-soft font-bold uppercase tracking-widest text-xs mt-1">Gestão de Produção e Insumos</p>
         </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 md:gap-6 auto-rows-[124px] sm:auto-rows-[140px]">
         ...
         >
            ...
            <div className="relative z-10 h-full flex flex-col justify-between">
               ...
               <div>
                  ...
                  <p className="text-muted font-medium text-sm max-w-xs">Controle de lotes de preparo, baixas de estoque automáticas e rotinas de pré-preparo matinais.</p>
               </div>
            </div>
         </button>
         ...
         <button onClick={() => router.push("/dashboard/operacao/cardapio")} className="col-span-2 row-span-1 rounded-[24px] p-4 sm:p-6 bg-card border border-line hover:border-slate-300 hover:shadow-md transition-all flex flex-col justify-between group">
            ...
            <div>
               ...
               <p className="text-3xs uppercase font-bold text-muted mt-0.5">Gestão de Itens e QR Code</p>
            </div>
         </button>

         <button onClick={() => router.push("/dashboard/operacao/fichas?dept=cozinha")} className="col-span-1 lg:col-span-2 row-span-1 rounded-[24px] p-4 sm:p-6 bg-card border border-line hover:border-slate-300 hover:shadow-md transition-all flex flex-col justify-between group">
         ...
         {/* ITENS NORMAIS (1x1) */}
         <button onClick={() => router.push("/dashboard/operacao/ingredientes?dept=cozinha")} className="col-span-1 row-span-1 rounded-[24px] p-5 bg-card border border-line hover:border-slate-300 transition-all flex flex-col justify-center items-center gap-2 group text-center">
         ...
         <button onClick={() => router.push("/dashboard/operacao/estoque?dept=cozinha")} className="col-span-1 row-span-1 rounded-[24px] p-5 bg-card border border-line hover:border-slate-300 transition-all flex flex-col justify-center items-center gap-2 group text-center">
         ...
         <button onClick={() => router.push("/dashboard/operacao/produtos?dept=cozinha")} className="col-span-1 row-span-1 rounded-[24px] p-5 bg-card border border-line hover:border-slate-300 transition-all flex flex-col justify-center items-center gap-2 group text-center">
            <PlusCircle size={24} className="text-emerald-500 group-hover:scale-110 transition-transform" />
            <h3 className="font-bold text-fg-soft text-sm leading-tight">Criação de Pratos</h3>
         </button>

         <button onClick={() => router.push("/dashboard/operacao/engenharia?dept=cozinha")} className="col-span-1 row-span-1 rounded-[24px] p-5 bg-card border border-line hover:border-slate-300 transition-all flex flex-col justify-center items-center gap-2 group text-center">
         ...
         <div className="col-span-2 lg:col-span-5 flex flex-col md:flex-row gap-4 mt-2">
            ...
            <button onClick={() => router.push("/dashboard/operacao/rotina?dept=cozinha&tipo=operacional")} className="flex-1 rounded-[20px] p-5 bg-slate-50 border border-line hover:bg-elevated transition-all flex items-center gap-4 group">
               ...
               <div className="text-left"><p className="font-bold text-indigo-900">Checklist Operacional</p><p className="text-3xs uppercase font-bold text-muted">Abertura e Fechamento</p></div>
            </button>

            <button onClick={() => router.push("/dashboard/operacao/rotina?dept=cozinha&tipo=limpeza")} className="flex-1 rounded-[20px] p-5 bg-slate-50 border border-line hover:bg-elevated transition-all flex items-center gap-4 group">
