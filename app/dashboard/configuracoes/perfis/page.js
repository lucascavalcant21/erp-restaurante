  ...
  return <div className="mx-auto w-full max-w-7xl p-4 sm:p-6 lg:p-8">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {profiles.map(profile=>{
        ...
        return <article key={profile.id} className={`rounded-2xl border bg-white p-5 shadow-sm ${profile.ativo?"border-slate-200":"border-slate-100 opacity-60"}`}><div className="mb-4 flex items-start justify-between gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 text-violet-600"><ShieldCheck size={19}/></div><span className={`rounded-full px-2 py-1 text-3xs font-bold uppercase ${profile.ativo?"bg-emerald-50 text-emerald-700":"bg-slate-100 text-slate-500"}`}>{profile.ativo?"Ativo":"Inativo"}</span></div><h2 className="font-black text-slate-800">{profile.nome}</h2><p className="mt-1 min-h-10 text-sm text-slate-500">{profile.descricao||"Sem descrição."}</p><div className="my-4 flex gap-4 border-y border-slate-100 py-3 text-xs font-bold text-slate-500"><span>{profile.permissions.length} regras</span><span>{count} usuários</span>{profile.version>1&&<span>Versão {profile.version}</span>}</div><div className="flex flex-wrap gap-1">
    ...
    {editing&&<Modal wide title={editing.id?`Editar ${editing.nome}`:"Novo perfil"} onClose={()=>setEditing(null)}><div className="grid gap-3 border-b p-4 sm:grid-cols-3"><div><label className="mb-1 block text-xs font-bold uppercase text-slate-400">Nome</label><input value={editing.nome} onChange={e=>setEditing({...editing,nome:e.target.value})} className={INPUT}/></div><div><label className="mb-1 block text-xs font-bold uppercase text-slate-400">Tipo</label><select value={editing.tipo} onChange={e=>setEditing({...editing,tipo:e.target.value})} className={INPUT}><option value="personalizado">Personalizado</option><option value="administrador">Administrador</option><option value="gerente">Gerente</option><option value="supervisor">Supervisor</option><option value="funcionario">Funcionário</option><option value="consulta">Consulta</option><option value="terminal_ponto">Terminal de ponto</option></select></div><div><label className="mb-1 block text-xs font-bold uppercase text-slate-400">Descrição</label><input value={editing.descricao||""} onChange={e=>setEditing({...editing,descricao:e.target.value})} className={INPUT}/></div></div><div className="p-4"><PermissionBuilder value={permissions} onChange={setPermissions} copySources={((data?.profiles || [])||[]).filter(p=>p.id!==editing.id).map(p=>({id:p.id,label:`Perfil: ${p.nome}`,permissions:p.permissions}))}/></div><footer className="flex justify-end gap-2 border-t p-4"><button onClick={()=>setEditing(null)} className="px-4 text-sm font-bold text-slate-500">Cancelar</button><button disabled={saving} onClick={save} className="rounded-xl bg-emerald-600 px-5 py-2 text-sm font-black text-white">{saving?"Salvando...":"Salvar perfil"}</button></footer></Modal>}
