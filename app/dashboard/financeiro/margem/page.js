...
const BCG_STYLE = {
  ...
  dog:      { label: "🐶 Abacaxi",       bg: "bg-rose-50",    borda: "border-rose-200",   cor: "#f43f5e", desc: "MC baixa + volume baixo" },
};
...
function CardMargem({ prato, rank }) {
  ...
  return (
    <div className={`rounded-2xl border shadow-sm overflow-hidden ${bcg === "dog" ? `${st.bg} ${st.borda}` : "bg-white border-neutral-100"}`}>
      <div className="px-4 pt-4 pb-3">
        <div className="flex items-start gap-3 mb-3">
          {/* Rank */}
          <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 font-black text-sm ${rank <= 3 ? "bg-amber-100 text-amber-700" : "bg-neutral-100 text-neutral-500"}`}>
            {rank <= 3 ? ["🥇","🥈","🥉"][rank - 1] : `#${rank}`}
          </div>
          ...
          <div className="text-right flex-shrink-0">
            ...
            <p className="text-[10px] font-bold text-neutral-400">MC</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-2">
          <div className="bg-neutral-50 rounded-xl px-2 py-2 text-center">
            <p className="text-[9px] font-black text-neutral-400 uppercase">Preço</p>
            <p className="text-[11px] font-black text-neutral-900">{fmtBRL(prato.preco)}</p>
          </div>
          <div className="bg-neutral-50 rounded-xl px-2 py-2 text-center">
            <p className="text-[9px] font-black text-neutral-400 uppercase">Custo</p>
            <p className="text-[11px] font-black text-neutral-900">{fmtBRL(prato.custo)}</p>
          </div>
          <div className="bg-neutral-50 rounded-xl px-2 py-2 text-center">
            <p className="text-[9px] font-black text-neutral-400 uppercase">MC/un</p>
            <p className="text-[11px] font-black text-emerald-700">{fmtBRL(mc_reais)}</p>
          </div>
          <div className="bg-neutral-50 rounded-xl px-2 py-2 text-center">
            <p className="text-[9px] font-black text-neutral-400 uppercase">Lucro/mês</p>
      ...
      <div className="sticky top-0 z-20 bg-[#fbf9f5] border-b border-neutral-200 px-4 pt-12 pb-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-black text-neutral-900 leading-tight">Margem de Lucro</h1>
            <p className="text-[11px] text-neutral-400 font-medium">Ranking de lucratividade por prato</p>
          </div>
          <button
      ...
      <div className="px-4 pt-4 pb-28 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-white border border-neutral-100 rounded-2xl shadow-sm p-4">
            ...
            <p className="text-xl font-black text-neutral-900">{fmtBRL(resumo.total_lucro)}</p>
            <p className="text-[11px] font-bold text-neutral-400">Lucro Total / Mês</p>
          </div>
          ...
          <div className={`rounded-2xl border p-4 ${resumo.dogs > 0 ? "bg-rose-50 border-rose-200" : "bg-white border-neutral-100"}`}>
            <p className={`text-[10px] font-black uppercase tracking-wider mb-1 ${resumo.dogs > 0 ? "text-rose-500" : "text-neutral-400"}`}>🐶 Abacaxis</p>
            <p className={`text-2xl font-black ${resumo.dogs > 0 ? "text-rose-700" : "text-neutral-900"}`}>{resumo.dogs}</p>
            <p className={`text-[10px] font-bold ${resumo.dogs > 0 ? "text-rose-500" : "text-neutral-400"}`}>pratos a revisar</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-4">
          <p className="text-[10px] font-black text-neutral-400 uppercase tracking-wider mb-3">Classificação da Matriz</p>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(BCG_STYLE).map(([k, v]) => (
              <div key={k} className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-[11px] font-black text-neutral-800">{v.label}</p>
                  <p className="text-[10px] text-neutral-400 font-medium">{v.desc}</p>
                </div>
              </div>
            ))}
          </div>
