...
function EtiquetaPapel({ item, responsavel, unidadeInfo, momento, tamanho, tipoEtiqueta }) {
  ...
  if (item.modeloEtiqueta === "nome") {
    const tamanhoNome = item.nome.length > 32 ? dim.titulo * 1.45 : item.nome.length > 20 ? dim.titulo * 1.7 : dim.titulo * 2.15;
    return <div className="etiqueta-rapida-papel etiqueta-somente-nome" style={{ width: `${dim.w}mm`, height: `${dim.h}mm`, padding: `${dim.pad + 1}mm`, background: "#fff", color: "#000", fontFamily: "Arial,Helvetica,sans-serif", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
      <div style={{ width: "100%", fontSize: `${tamanhoNome}mm`, lineHeight: 1.05, fontWeight: 950, textAlign: "center", textTransform: "uppercase", overflowWrap: "anywhere" }}>{item.nome}</div>
    </div>;
  }
...
export default function EtiquetasRapidas() {
  ...
  async function salvarFilaComoLista() {
    ...
    const nome = window.prompt("Nome para identificar esta lista:", padrao);
    if (nome === null) return;
  ...
  return <div className="etq-app" style={{ "--cor": setor === "bar" ? "#2563eb" : "#059669", "--suave": setor === "bar" ? "#eff6ff" : "#ecfdf5" }}>
    ...
    <main className="etq-conteudo">
      ...
      {listasSalvas.length > 0 && <section className="etq-listas-salvas"><div className="etq-listas-titulo"><FolderOpen size={19}/><div><strong>Listas salvas</strong><span>Carregue uma lista e imprima novamente</span></div></div><div className="etq-listas-grid">{listasSalvas.map(lista => <article key={lista.id}><button className="abrir" onClick={() => carregarListaSalva(lista)}><strong>{lista.nome}</strong><small>{lista.total_etiquetas || (lista.itens || []).reduce((soma, itemLista) => soma + numero(itemLista.copias), 0)} etiqueta(s) · {lista.responsavel_nome || "Equipe"}</small></button><button className="excluir" onClick={() => removerListaSalva(lista)} aria-label={`Excluir ${lista.nome}`}><Trash2 size={16}/></button></article>)}</div></section>}
