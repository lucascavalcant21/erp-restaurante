...
function NovaReservaModal({ onClose, onSuccess, unidadeAtiva, userSession }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await supabase.from("reservas").insert([{
      ...
      responsavel_id: userSession?.user?.id || null,
      responsavel_nome: userSession?.user?.user_metadata?.full_name || "Staff"
    }]);
    setSalvando(false);
    if (!error) {
      onSuccess();
...
let t = fs.readFileSync('app/dashboard/reservas-eventos/reservas/page.js', 'utf-8');

// Insert NovaReservaModal component at the end
t = t + '\n' + modalCode;

// Inject modal state inside ReservasPage
t = t.replace('const [carregando, setCarregando] = useState(true);', 
              'const [carregando, setCarregando] = useState(true);\n  const [modalAberto, setModalAberto] = useState(false);\n  const { user } = useERP();');

// Bind button to open modal
t = t.replace('<button className="h-11 px-5 rounded-xl bg-slate-900 text-white font-bold flex items-center gap-2 hover:bg-slate-800">', 
...
// Add modal render at the end of the return
t = t.replace('</main>', `      {modalAberto && (
        <NovaReservaModal 
...
// Fix carregarReservas scope so it can be called from onSuccess
t = t.replace('useEffect(() => {\n    async function carregarReservas() {', 
              'async function carregarReservas() {\n      if (!unidadeAtiva) return;\n      setCarregando(true);\n      const { data, error } = await supabase\n        .from("reservas")\n        .select("*")\n        .eq("unidade_id", unidadeAtiva)\n        .order("data_reserva", { ascending: true })\n        .order("horario", { ascending: true });\n        \n      if (!error && data) setReservas(data);\n      setCarregando(false);\n    }\n\n    useEffect(() => {\n      carregarReservas();\n    ');

// Remove the inner implementation of carregarReservas that was duplicated
t = t.replace('useEffect(() => {\n      carregarReservas();\n    \n      if (!unidadeAtiva) return;\n      setCarregando(true);\n      const { data, error } = await supabase\n        .from("reservas")\n        .select("*")\n        .eq("unidade_id", unidadeAtiva)\n        .order("data_reserva", { ascending: true })\n        .order("horario", { ascending: true });\n        \n      if (!error && data) setReservas(data);\n      setCarregando(false);\n    }\n    carregarReservas();\n  }, [unidadeAtiva]);', 
              'useEffect(() => {\n      carregarReservas();\n  }, [unidadeAtiva]);');

// Just to be safe, I'll rewrite the entire file properly.
fs.writeFileSync('rewrite-reservas.js', \`
const fs = require('fs');
...
fs.writeFileSync('app/dashboard/reservas-eventos/reservas/page.js', code, 'utf-8');
console.log('Done!');
\`
