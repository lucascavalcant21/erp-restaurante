    ...
    <div className="mx-auto w-full max-w-[95rem] p-4 sm:p-6 lg:p-8">
      <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><p className="text-xs font-bold uppercase tracking-[.16em] text-success">Configurações</p><h1 className="text-2xl font-black text-slate-800">Usuários e acessos</h1><p className="text-sm text-muted">Login, escopos, segurança e permissões individuais.</p></div>
        </div>
        <button onClick={() => openForm()} className="flex items-center justify-center gap-2 rounded-xl bg-accent px-5 py-3 text-sm font-black text-accent-fg shadow-lg shadow-emerald-200"><Plus size={17}/> Novo usuário</button>
      </header>
      ...
      {notice && <div className="mb-4 flex items-center justify-between rounded-xl bg-accent-soft px-4 py-3 text-sm font-bold text-accent-strong">{notice}<button onClick={() => setNotice("")}><X size={16}/></button></div>}
              ...
              {users.map((user) => {
                ...
                return <tr key={user.id} className="text-sm text-slate-600 hover:bg-slate-50/70">
                  ...
                  <td className="px-4 py-3"><div className="flex gap-1">
                    ...
                    {user.status==="ativo"?<button title="Bloquear" onClick={()=>status(user,"bloqueado")} className="rounded-lg p-2 text-amber-600 hover:bg-amber-50"><Lock size={15}/></button>:<button title="Desbloquear" onClick={()=>status(user,"ativo")} className="rounded-lg p-2 text-accent-strong hover:bg-accent-soft"><CheckCircle2 size={15}/></button>}
      ...
      {modal==="form" && <Modal title={form.id?"Editar usuário":"Novo usuário"} subtitle="Dados pessoais, vínculo, escopo e políticas de segurança." onClose={()=>setModal(null)}>
        <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2"><label className={LABEL}>Dias permitidos</label><div className="flex gap-2">{DAYS.map(([day,label])=><button type="button" key={day} onClick={()=>{const n=Number(day),days=form.allowed_days.includes(n)?form.allowed_days.filter(x=>x!==n):[...form.allowed_days,n];setForm({...form,allowed_days:days})}} className={`h-9 w-9 rounded-lg text-xs font-bold ${form.allowed_days.includes(Number(day))?"bg-accent text-accent-fg":"bg-elevated text-subtle"}`}>{label}</button>)}</div></div>
