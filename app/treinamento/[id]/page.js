...
export default function TreinamentoPublico() {
  ...
  if (loading) return <div className="grid min-h-screen place-items-center bg-elevated font-bold text-muted">Abrindo treinamento...</div>;
  if (!item) return <div className="grid min-h-screen place-items-center bg-elevated p-6 text-center"><div><BookOpen size={48} className="mx-auto text-dim"/><h1 className="mt-3 text-2xl font-black">Treinamento não encontrado</h1></div></div>;
  ...
  return (
    <main className="min-h-screen bg-elevated px-4 py-6 text-fg sm:py-10">
      <article className="mx-auto max-w-3xl overflow-hidden rounded-3xl bg-card shadow-xl ring-1 ring-slate-200">
        <header className="bg-slate-950 p-6 text-white sm:p-9">
          ...
          {item.descricao && <p className="mt-3 text-base font-medium text-dim sm:text-lg">{item.descricao}</p>}
        </header>
