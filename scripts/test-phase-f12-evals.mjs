...
console.log("==================================================");
console.log("   HÉFISTO FASE F12 — COPILOTO CONTEXTUAL EVALS");
console.log("==================================================\n");

async function runTestF12Evals() {
  console.log("--- 1. Evaluating Dataset Composition ---");
  console.log(` Dataset size: ${EVAL_DATASET.length} total synthetic test cases.`);
  if (EVAL_DATASET.length < 130) {
    console.error(`❌ ERROR: Dataset contains ${EVAL_DATASET.length} cases (must be >= 130).`);
    process.exit(1);
  }
  ...
  console.log("\n==================================================");
  console.log("          HEFESTO F12 EVAL REPORT                 ");
  console.log("==================================================");
  ...
  if (fullReport.overallStatus !== "PASS" || fullReport.criticalSafetyPass !== "PASS") {
    console.error("❌ ERROR: HEFESTO F12 EVAL REPORT FAILED OVERALL STATUS OR CRITICAL SAFETY GATE!");
    process.exit(1);
  }

  console.log("✅ SUCCESS: HÉFISTO FASE F12 — COPILOTO CONTEXTUAL EVALS FULLY PASSED AND VERIFIED.");
}

runTestF12Evals().catch(err => {
  console.error("❌ UNHANDLED EXCEPTION IN F12 EVALS SUITE:", err);
  process.exit(1);
});
