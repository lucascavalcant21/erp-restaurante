...
export default function AtualizarPerfilRH() {
  ...
  if (carregando) return <main className="min-h-screen bg-slate-50 grid place-items-center"><div className="text-center"><Loader2 className="mx-auto animate-spin text-emerald-600"/><p className="mt-3 font-bold text-muted">Abrindo seu cadastro...</p></div></main>;
  ...
  return <main className="min-h-screen bg-elevated px-3 py-6 sm:px-6 sm:py-10"><form onSubmit={salvar} className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-line bg-card shadow-xl">
    ...
    <div className="space-y-7 p-5 sm:p-8">
      ...
      </div>{perfil.tem_filhos && <div className="mt-5"><div className="mb-2 flex items-center justify-between"><p className="text-xs font-bold text-slate-600">Filhos / dependentes <b className="text-rose-500">*</b></p><button type="button" onClick={()=>set("filhos",[...(perfil.filhos||[]),{nome:"",data_nascimento:""}])} className="flex items-center gap-1 text-xs font-bold text-emerald-700"><Plus size={14}/> Adicionar</button></div><div className="space-y-2">{(perfil.filhos||[]).map((filho,i)=><div key={i} className="grid grid-cols-[1fr_140px_36px] gap-2"><input required value={filho.nome||""} onChange={e=>set("filhos",perfil.filhos.map((x,j)=>j===i?{...x,nome:e.target.value}:x))} placeholder="Nome" className={INPUT.replace("mt-1.5 ","")}/><input required type="date" value={String(filho.data_nascimento||"").slice(0,10)} onChange={e=>set("filhos",perfil.filhos.map((x,j)=>j===i?{...x,data_nascimento:e.target.value}:x))} className={INPUT.replace("mt-1.5 ","")}/><button type="button" onClick={()=>set("filhos",perfil.filhos.filter((_,j)=>j!==i))} className="grid place-items-center rounded-xl bg-rose-50 text-rose-600"><Trash2 size={16}/></button></div>)}</div></div>}</section>
      <p className="flex items-start gap-2 rounded-xl bg-slate-50 p-3 text-xs font-medium text-muted"><ShieldCheck size={16} className="shrink-0 text-emerald-600"/>Todos os campos com * são obrigatórios. Este formulário não mostra salário, avaliações ou histórico. O link é pessoal e vence automaticamente.</p>
    </div><footer className="sticky bottom-0 border-t border-line bg-white/95 p-4 backdrop-blur sm:px-8"><button disabled={salvando} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 font-black text-white shadow-lg shadow-emerald-700/20 disabled:opacity-60">{salvando?<Loader2 className="animate-spin" size={19}/>:<Save size={19}/>} {salvando?"Salvando...":"Salvar meus dados"}</button></footer>
  </form></main>;
}
