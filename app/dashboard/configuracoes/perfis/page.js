  ...
  return <div className="mx-auto w-full max-w-7xl p-4 sm:p-6 lg:p-8">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {profiles.map(profile=>{
        ...
        return <article key={profile.id} className={`rounded-2xl border bg-card p-5 shadow-sm ${profile.ativo?"border-line":"border-line-soft opacity-60"}`}><div className="mb-4 flex items-start justify-between gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-50 text-violet-600"><ShieldCheck size={19}/></div><span className={`rounded-full px-2 py-1 text-3xs font-bold uppercase ${profile.ativo?"bg-emerald-50 text-emerald-700":"bg-elevated text-muted"}`}>{profile.ativo?"Ativo":"Inativo"}</span></div><h2 className="font-black text-slate-800">{profile.nome}</h2><p className="mt-1 min-h-10 text-sm text-muted">{profile.descricao||"Sem descrição."}</p><div className="my-4 flex gap-4 border-y border-line-soft py-3 text-xs font-bold text-muted"><span>{profile.permissions.length} regras</span><span>{count} usuários</span>{profile.version>1&&<span>Versão {profile.version}</span>}</div><div className="flex flex-wrap gap-1">
