...
export default function Skeleton({ variant = "card", count = 3, rows = 5, columns = 3 }) {
    ...
    return (
      <div className="space-y-3">
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="bg-card rounded-2xl p-4 shadow-sm border border-neutral-100 animate-pulse">
  ...
  if (variant === "list") {
    return (
      <div className="space-y-2">
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="bg-card rounded-xl px-4 py-3 border border-neutral-100 animate-pulse flex items-center gap-3">
  ...
  if (variant === "table") {
    return (
      <div className="bg-card rounded-2xl border border-neutral-100 shadow-sm overflow-hidden animate-pulse">
        {/* Header */}
  ...
  if (variant === "kpi") {
    return (
      <div className="grid grid-cols-2 gap-3">
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="bg-card rounded-2xl border border-neutral-100 shadow-sm p-3 animate-pulse">
